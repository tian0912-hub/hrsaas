    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <image-upload ref="employeeImage" />
        </el-form-item>
...
import { saveEmployeeInfo, getPersonalDetail, updatePersonal } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      this.$refs.employeeImage.fileList = [{ url: this.formData.staffPhoto, upload: true }]
    },
    async saveUser() {
      const fileList = this.$refs.staffPhoto.fileList
      if (fileList.some(item => !item.upload)) {
        return this.$message.error('图片还没上传成功')
      }
      this.userInfo = { ...this.userInfo, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' }
      await saveEmployeeInfo(this.userInfo)
      this.$message.success('更新用户基本信息成功')
    },
    async savePersonal() {
      const fileList = this.$refs.employeeImage.fileList
      if (fileList.some(item => !item.upload)) {
        return this.$message.error('图片还没上传成功')
      }
      this.formData = { ...this.formData, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' }
