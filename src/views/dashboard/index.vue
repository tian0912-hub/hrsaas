  ...
  <div class="dashboard-container">
    ...
    <el-card class="header-card">
      <div>
        <div class="fl headL">
          <div class="headImg">
            <img v-imagerror="defaultImg" :src="imgUrl">
          </div>
          <div class="headInfoTip">
            <p class="firstChild">早安，{{ $store.state.user.userInfo.username }}，祝你开心每一天！</p>
            <p class="lastChild">早安，{{ $store.state.user.userInfo.username }} | {{ $store.state.user.userInfo.companyName }}-{{ $store.state.user.userInfo.departmentName }}</p>
          </div>
        </div>
        <div class="fr" />
      </div>
    ...
    <el-row type="flex" justify="space-between">
      ...
      <el-col :span="13" style="padding-right:26px">
        ...
        <el-card class="box-card">
          ...
          <!-- 放置日历组件 -->
          <workCalendar :start-date.sync="startDate" />
        </el-card>
        <!-- 公告 -->
        <el-card class="box-card">
...
export default {
  ...
  data() {
    return {
      imgUrl: this.$store.state.user.userInfo.staffPhoto,
      defaultImg: require('@/assets/common/head.jpg'),
      startDate: new Date(),
      showDialog: false, // 审批申请弹窗
      ruleForm: {
        exceptTime: '',
        reason: '',
